#!/usr/bin/env bash
# Append one section per build run to docs/aosp-build-history.md and
# commit it. One run = one commit, in chronological order.
#
# Intentionally ASCII-only (no em-dashes) so the resulting commits work
# on git-bash / cmd / powershell consoles without re-encoding issues.
set -euo pipefail
DOC=docs/aosp-build-history.md
cd "$(git rev-parse --show-toplevel)"

append_and_commit() {
    local run="$1" subject="$2" body="$3"
    {
        echo
        echo "## Run $run"
        echo
    ...
    git add "$DOC"
    git commit -q -m "docs(build-history): run $run - $subject"
}

append_and_commit 1 "SIGHUP, spawn fixed via setsid" \
"**Phase**: process spawn
...
append_and_commit 2 "cortex-a510 unsupported by AOSP 14 clang" \
...
append_and_commit 3 "Android.bp lib variant mis-shape" \
...
append_and_commit 4 "external/mesa3d Soong namespace missing" \
"**Phase**: Kati legacy parse
...
append_and_commit 5 "killed by power outage at 12 min" \
"**Phase**: ninja early
...
append_and_commit 6 "VINTF in PRODUCT_COPY_FILES forbidden" \
"**Phase**: Kati late
...
append_and_commit 7 "BOARD_AVB_VBMETA_*_ROLLBACK_INDEX_LOCATION undefined" \
"**Phase**: Kati late
...
append_and_commit 8 "duplicate vendor/etc/vintf/manifest.xml install rule" \
...
append_and_commit 9 "soong_build OOM-killed at 15 GB RSS" \
...
append_and_commit 10 "microG Android.mk writes into source tree" \
"**Phase**: Kati 100%
...
append_and_commit 11 "kernel binary missing, no rule to make it" \
"**Phase**: ninja early
...
append_and_commit 12 "first run with correct kernel install path" \
"**Phase**: ninja (real compile)
**Outcome**: ran clean for the first time. Compile phase begun.
**Fix for run 13**: none; this run was healthy."

append_and_commit 13 "power outage at 36 percent (46,947/127,283 steps)" \
...
append_and_commit 14 "turbine ZipException invalid zip archive (513 corrupt)" \
"**Phase**: ninja mid-compile
...
append_and_commit 15 "power outage at 5 percent (4,809/85,081 steps)" \
"**Phase**: ninja early
...
append_and_commit 16 "brief test of new watchdogs (no real progress)" \
"**Phase**: re-spawn check
**Outcome**: sanity-restart confirmed sync_loop + hotkey wiring before further iteration.
**Fix for run 17**: none."

append_and_commit 17 "post-deep-sweep restart, 526 host-tool jars corrupt" \
...
append_and_commit 18 "verification sweep, 0 corrupt" \
"**Phase**: pre-flight
**Outcome**: round-4 sweep scanned 8,521 archives, found 0 corrupt. Tree confirmed clean before run 19.
**Fix for run 19**: none."

append_and_commit 19 "sepolicy version 33.0 vs board 202404" \
...
append_and_commit 20 "kernel target-level equals device target-level" \
"**Phase**: ninja late
...
append_and_commit 21 "hal_telephony_default_exec undefined in AOSP 14" \
"**Phase**: ninja late
...
append_and_commit 22 "fileslist panic on missing vendor_ramdisk directory" \
...
append_and_commit 23 "mkbootimg --vendor_boot needs --header_version" \
...
append_and_commit 24 "HAL declared in both manifest.xml and vintf_fragment" \
...
append_and_commit 25 "health@2.1 and radio@1.6 deprecated in FCM 7" \
...
append_and_commit 26 "cas IMediaCasService not declared at FCM 6" \
"**Phase**: ninja very late
...
append_and_commit 27 "same check_vintf_compatible error after fragment drop" \
"**Phase**: ninja very late
...
append_and_commit 28 "PRODUCT_ENFORCE_VINTF_MANIFEST_OVERRIDE missing" \
"**Phase**: ninja very late
...
append_and_commit 29 "SUCCESS — all 5 images produced" \
...
echo
echo "=== all 29 build runs committed ==="
git log --oneline --grep="docs(build-history)" | wc -l
