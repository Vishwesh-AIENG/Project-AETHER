...
| AT-2 | ✅ **Lift done** | Full lift bodies for every major DecodedInsn variant: PC-rel, integer ALU (imm/reg/extended), MOVZ/N/K, BFM, EXTR, CSEL, MADD/MSUB, UDIV/SDIV, shifts, DP-1-src, CCMP, CRC32, all load/store forms (incl. pair, LL/SC, LSE atomics, CAS, SWP, LDAR/STLR/LDAPR), branches (B/BL/B.cond/CBZ/CBNZ/TBZ/TBNZ/BR/BLR/RET), exception-gen (SVC/HVC/SMC/BRK/HLT), hints (NOP/YIELD/WFE/WFI/SEV/SEVL/PAC/BTI), barriers (DMB/DSB/ISB/SB), MSR/MRS with named-sysreg lookup, MSR-imm, SYS IC/DC/AT/TLBI, UDF, and coarse opaque lifts for SIMD/FP/crypto (refined in Phase B). New IR ops: ReadGpr/WriteGpr/ReadSp/WriteSp/ReadFpr/WriteFpr/ReadFlags/WriteFlags/ReadPc/WritePc — pre-SSA register access plumbing. Phase B SSA construction folds these into proper SSA. |
