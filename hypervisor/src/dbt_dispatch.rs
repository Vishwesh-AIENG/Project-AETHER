// dbt_dispatch.rs — FEX VMEXIT dispatch loop (x86 tier).
//
...
use crate::android_handoff::DbtInitialRegs;
use crate::mmio_emu::{self, MmioAccess, MmioResult};

// FFI surface is always live now — `fex_*` Rust functions in
// `dbt_integration.rs` route through the aether-translator crate; no
// libfex.a static archive is required.
use crate::dbt_integration::{aether_dbt_dispatch_block_hv, aether_dbt_translate_block_hv};

// ─────────────────────────────────────────────────────────────────────────────
// DbtMmioRequest — FEX-populated record describing one trapped ARM64 MMIO
//
...
// well-known location (see `AETHER_DBT_MMIO_REQUEST` below) and emulates
...
//     3. Fill `AETHER_DBT_MMIO_REQUEST` with:
...
static mut AETHER_DBT_MMIO_REQUEST: DbtMmioRequest = DbtMmioRequest::empty();

/// Read the request slot. The caller should clear `valid` after consuming.
pub fn current_mmio_request() -> DbtMmioRequest {
    ...
    unsafe {
        let p = core::ptr::addr_of!(AETHER_DBT_MMIO_REQUEST);
        *p
    }
}

/// Consume the request slot: read it, then reset `valid` to false.
pub fn take_mmio_request() -> DbtMmioRequest {
    let r = current_mmio_request();
    // SAFETY: see above.
    unsafe {
        let p = core::ptr::addr_of_mut!(AETHER_DBT_MMIO_REQUEST);
        (*p).valid = false;
    }
    r
}
...
#[cfg(test)]
pub fn set_mmio_request_for_test(req: DbtMmioRequest) {
    unsafe {
        let p = core::ptr::addr_of_mut!(AETHER_DBT_MMIO_REQUEST);
        *p = req;
    }
...
// ─────────────────────────────────────────────────────────────────────────────
// DbtDispatchState — single-instance EL2-global FEX state
// ─────────────────────────────────────────────────────────────────────────────

/// One iteration's worth of statistics + state mutation tracking.
#[derive(Debug, Clone, Copy, Default)]
pub struct DbtDispatchStats {
    /// Number of times `aether_dbt_translate_block_hv` was attempted.
    pub blocks_translated: u64,
    /// Number of times `aether_dbt_dispatch_block_hv` was attempted.
...
pub struct DbtDispatchState {
    ...
    pub stats: DbtDispatchStats,
}

impl DbtDispatchState {
    pub const fn new() -> Self {
        Self {
            regs:  DbtInitialRegs::zero(),
            pc:    0,
            armed: false,
            stats: DbtDispatchStats {
    ...
    /// translate.
    pub fn arm(&mut self, regs: DbtInitialRegs) {
        self.regs = regs;
        self.pc   = regs.pc;
        self.armed = true;
    }
...
pub fn classify_intel(exit_reason: u32, exit_qualification: u64, gpa: u64) -> DbtExitClass {
    use crate::vtx;
    let _ = exit_qualification;
    match exit_reason {
        vtx::EXIT_REASON_HLT             => DbtExitClass::Halt,
        vtx::EXIT_REASON_EPT_VIOLATION   => DbtExitClass::Mmio { addr: gpa },
        vtx::EXIT_REASON_CPUID           => DbtExitClass::Translate,
        vtx::EXIT_REASON_EXCEPTION_NMI   => DbtExitClass::Translate, // #UD from FEX-translated code
        vtx::EXIT_REASON_EXTERNAL_IRQ    => DbtExitClass::Translate, // host IRQ window — re-enter
        _                                => DbtExitClass::Unhandled,
    }
}

/// Decode an AMD VMCB exit_code into an exit class. NPF (nested page fault)
/// is the AMD analogue of EPT_VIOLATION.
pub fn classify_amd(exit_code: u64, npf_gpa: u64) -> DbtExitClass {
    use crate::svm;
    match exit_code {
        svm::SVM_EXIT_HLT       => DbtExitClass::Halt,
        0x400 /* NPF */         => DbtExitClass::Mmio { addr: npf_gpa },
        svm::SVM_EXIT_INTR      => DbtExitClass::Translate,
        svm::SVM_EXIT_IOIO      => DbtExitClass::IoPort,
        svm::SVM_EXIT_CPUID     => DbtExitClass::Translate,
        _                       => DbtExitClass::Unhandled,
    }
}
...
pub fn dispatch_one_block(state: &mut DbtDispatchState) -> Result<(), DispatchError> {
    if !state.armed {
    ...
    let mut len:    u32 = 0;
    // SAFETY: Rust function with valid out-pointers; the adapter in
    // dbt_integration.rs forwards to aether_translator::dbt.
    let r = unsafe { aether_dbt_translate_block_hv(state.pc, &mut host_pa, &mut len) };
    if !r.is_ok() {
        return Err(DispatchError::TranslateFailed);
    }
    state.stats.blocks_dispatched += 1;
    // DbtThreadHandle is opaque; the translator ignores it (block cache is
    // keyed on guest PC, not thread).
    let null_thread: crate::dbt_integration::DbtThreadHandle = core::ptr::null_mut();
    let r = unsafe { aether_dbt_dispatch_block_hv(null_thread, host_pa) };
    if !r.is_ok() {
        return Err(DispatchError::DispatchFailed);
    }
    // Step 1 keeps a fixed 4-byte PC advance per block (one ARM64 insn).
    // Step 2 of the integration plan will read the real advance from the
    // translator's GuestRegisterFile after dispatch.
...
pub enum DispatchError {
    ...
    NotArmed,
    /// FEX returned non-Ok from `aether_dbt_translate_block_hv`.
    TranslateFailed,
    /// FEX returned non-Ok from `aether_dbt_dispatch_block_hv`.
    DispatchFailed,
}
...
/// value).
pub fn handle_mmio(state: &mut DbtDispatchState, access: MmioAccess, xt_index: u8) -> MmioResult {
...
/// exit fields and converted them into a `DbtExitClass`.
pub fn handle_vmexit(state: &mut DbtDispatchState, exit: DbtExitClass) -> VmexitAction {
    match exit {
        DbtExitClass::Translate => {
            match dispatch_one_block(state) {
                Ok(())  => VmexitAction::Reenter,
                Err(_)  => VmexitAction::Halt,
            }
        }
        DbtExitClass::Mmio { addr } => {
...
static mut AETHER_DBT_STATE: DbtDispatchState = DbtDispatchState::new();

/// # Safety
/// Must be called at EL2 single-core before the guest first executes.
pub unsafe fn arm_global(regs: DbtInitialRegs) {
    unsafe {
        let p = core::ptr::addr_of_mut!(AETHER_DBT_STATE);
        (*p).arm(regs);
    }
}

/// Run a closure with the global FEX state.
pub fn with_global_mut<R, F: FnOnce(&mut DbtDispatchState) -> R>(f: F) -> R {
    // SAFETY: see comment on `AETHER_DBT_STATE`.
    unsafe {
        let p = core::ptr::addr_of_mut!(AETHER_DBT_STATE);
        f(&mut *p)
    }
...
mod tests {
    ...
    fn armed_state(pc: u64) -> DbtDispatchState {
        let mut s = DbtDispatchState::new();
        s.arm(DbtInitialRegs::for_kernel_entry(pc, 0x4400_0000));
        s
    }

    #[test]
    fn new_state_is_not_armed() {
        let s = DbtDispatchState::new();
        assert!(!s.armed);
        assert_eq!(s.pc, 0);
    }
    ...
    fn classify_intel_routes_hlt_and_ept_violation() {
        use crate::vtx::*;
        assert_eq!(classify_intel(EXIT_REASON_HLT, 0, 0), DbtExitClass::Halt);
        assert_eq!(
            classify_intel(EXIT_REASON_EPT_VIOLATION, 0, 0x0900_0000),
            DbtExitClass::Mmio { addr: 0x0900_0000 },
        );
        assert_eq!(classify_intel(EXIT_REASON_CPUID, 0, 0), DbtExitClass::Translate);
        assert_eq!(classify_intel(0xDEAD, 0, 0), DbtExitClass::Unhandled);
    }

    #[test]
    fn classify_amd_routes_hlt_and_npf() {
        use crate::svm::*;
        assert_eq!(classify_amd(SVM_EXIT_HLT, 0), DbtExitClass::Halt);
        assert_eq!(
            classify_amd(0x400, 0x0800_0000),
            DbtExitClass::Mmio { addr: 0x0800_0000 },
        );
        assert_eq!(classify_amd(SVM_EXIT_CPUID, 0), DbtExitClass::Translate);
        assert_eq!(classify_amd(SVM_EXIT_IOIO, 0), DbtExitClass::IoPort);
    }

    #[test]
    fn vmexit_translate_reenter() {
        let mut s = armed_state(0x4080_0000);
        let act = handle_vmexit(&mut s, DbtExitClass::Translate);
    ...
    fn vmexit_halt_halts() {
        let mut s = armed_state(0x4080_0000);
        let act = handle_vmexit(&mut s, DbtExitClass::Halt);
    ...
    fn vmexit_unhandled_halts() {
        let mut s = armed_state(0x4080_0000);
        let act = handle_vmexit(&mut s, DbtExitClass::Unhandled);
        assert_eq!(act, VmexitAction::Halt);
    ...
    fn vmexit_unknown_mmio_addr_halts() {
        let mut s = armed_state(0x4080_0000);
        let act = handle_vmexit(&mut s, DbtExitClass::Mmio { addr: 0xDEAD_BEEF });
        assert_eq!(act, VmexitAction::Halt);
    ...
    fn vmexit_virtio_mmio_with_no_backend_halts() {
        ...
        let mut s = armed_state(0x4080_0000);
        let act = handle_vmexit(&mut s, DbtExitClass::Mmio { addr: VIRTIO_MMIO_BASE_IPA });
        assert_eq!(act, VmexitAction::Halt);
        assert_eq!(s.stats.mmio_unhandled, 1);
    }
    ...
    // ── Phase 5b: DbtMmioRequest contract ──────────────────────────────────

    #[test]
    fn mmio_request_empty_is_not_sane() {
        assert!(!DbtMmioRequest::empty().is_sane());
    }

    #[test]
    fn mmio_request_rejects_bad_size() {
        let req = DbtMmioRequest { valid: true, size: 3, xt: 0, ..Default::default() };
        assert!(!req.is_sane());
    }

    #[test]
    fn mmio_request_rejects_bad_xt() {
        let req = DbtMmioRequest { valid: true, size: 4, xt: 99, ..Default::default() };
        assert!(!req.is_sane());
    }

    #[test]
    fn mmio_request_xt31_is_sane_xzr_discard() {
        let req = DbtMmioRequest { valid: true, size: 4, xt: 31, ..Default::default() };
        assert!(req.is_sane());
    }
    ...
    fn vmexit_mmio_consumes_request_record_when_valid() {
        ...
        set_mmio_request_for_test(DbtMmioRequest {
            valid: true,
            is_write: true,
            size: 1,
            xt: 2,
        ...
        let act = handle_vmexit(
            &mut s,
            DbtExitClass::Mmio { addr: crate::mmio_emu::PL011_UART_BASE },
        );
        assert_eq!(act, VmexitAction::Reenter);
        assert_eq!(s.stats.mmio_handled, 1);
        // Slot consumed.
    ...
    fn vmexit_mmio_uses_fallback_synthesis_without_request() {
        ...
        set_mmio_request_for_test(DbtMmioRequest::empty());
        let mut s = armed_state(0x4080_0000);
        let act = handle_vmexit(
            &mut s,
            DbtExitClass::Mmio {
